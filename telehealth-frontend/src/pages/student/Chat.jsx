    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
        ...
        <textarea value={comments} onChange={e => setComments(e.target.value)}
          placeholder="Any additional comments? (optional)" rows={3}
        ...
        <div className="flex gap-3">
          <button onClick={onClose} className="flex-1 py-2.5 rounded-lg border border-gray-200 text-sm text-gray-500 cursor-pointer">Skip</button>
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
        ...
        <p className="text-sm text-gray-400 mb-6">Escalate to a voice, video, or in-person meeting.</p>
        <div className="flex gap-2 mb-5">
          {['VOICE', 'VIDEO', 'IN_PERSON'].map(t => (
            <button key={t} onClick={() => setType(t)}
              className={`flex-1 py-2 rounded-lg text-xs font-medium border transition cursor-pointer ${
                type === t ? 'border-[#003D8F] bg-[#E8F0FC] text-[#003D8F]' : 'border-gray-200 text-gray-400'
              }`}>{t.replace('_', ' ')}</button>
          ))}
        </div>
        ...
        <div className="flex gap-3">
          ...
          <button onClick={handleSubmit} disabled={loading}
            className="flex-1 py-2.5 rounded-lg bg-[#003D8F] text-white text-sm font-semibold disabled:opacity-60 cursor-pointer">
...
export default function StudentChat() {
  ...
  const handleCloseSession = async () => {
    ...
    try {
      ...
      setSessions(prev => prev.map(s => s.session_id === activeSession.session_id ? updated : s))
        ...
        {loadingSessions ? (
          ...
          <div className="flex flex-col items-center justify-center h-40 px-5 text-center">
            ...
            <button onClick={handleStartSession}
              className="mt-3 px-3 py-1.5 bg-[#003D8F] text-white text-xs font-semibold rounded-lg cursor-pointer">
              Start session
            </button>
          </div>
        ) : (
          sessions.map(session => (
            <button key={session.session_id} onClick={() => selectSession(session)}
              className={`w-full flex items-center gap-3 px-5 py-3.5 border-b border-gray-50 text-left transition cursor-pointer ${
                activeSession?.session_id === session.session_id ? 'bg-[#E8F0FC]' : 'hover:bg-gray-50'
              }`}>
  ...
  return (
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      ...
      {showFeedback && (
        <FeedbackModal sessionId={feedbackSessionId} onClose={() => setShowFeedback(false)} onSubmitted={handleFeedbackSubmitted} />
      )}
      {showAppointment && (
        <AppointmentModal sessionId={activeSession?.session_id} onClose={() => setShowAppointment(false)} onBooked={handleAppointmentBooked} />
      )}
    </div>
  )
}
