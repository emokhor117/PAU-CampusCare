...
export class EmailService {
  ...
  constructor() {
    ...
    this.logger.log('Email service ready via Brevo');
  }
  ...
  async sendStudentCredentials(opts: {
    ...
    try {
      ...
      sendSmtpEmail.subject = 'Your PAU CampusCare Account';
      ...
      this.logger.error(`Failed to send student credentials to ${opts.to}: ${err?.message || err}`);
    }
  }
  ...
  async sendCounsellorCredentials(opts: {
    ...
    try {
      ...
      sendSmtpEmail.sender = { email: this.fromEmail, name: this.fromName };
      sendSmtpEmail.to = [{ email: opts.to }];
      sendSmtpEmail.subject = 'Your PAU CampusCare Counsellor Account';
      sendSmtpEmail.htmlContent = `
        <div style="font-family: Arial, sans-serif; max-width: 560px; margin: 0 auto; padding: 32px; background: #f8fafc; border-radius: 12px;">
          <div style="background: #003D8F; border-radius: 8px; padding: 24px; text-align: center; margin-bottom: 24px;">
            <h1 style="color: white; margin: 0; font-size: 20px;">PAU CampusCare</h1>
      ...
      this.logger.error(`Failed to send counsellor credentials to ${opts.to}: ${err?.message || err}`);
    }
  }
  ...
  async sendSessionNotification(opts: {
    ...
    try {
      ...
      sendSmtpEmail.sender = { email: this.fromEmail, name: this.fromName };
      sendSmtpEmail.to = [{ email: opts.to }];
      sendSmtpEmail.subject = opts.is_priority
